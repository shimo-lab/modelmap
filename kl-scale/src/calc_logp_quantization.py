...
def parse_args():
    parser = ArgumentParser(description="Calculate logp values for quantized models.")
    parser.add_argument(
        "--model-list-path",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<repo>/output/logp/<bit>bit_quantization_logp.pkl."
        ),
    )
    parser.add_argument(
        "--overwrite",
    ...
    parser.add_argument(
        ...
        choices=(4, 8),
        default=4,
        help="BitsAndBytes quantization bit width. Defaults to 4.",
    )
    parser.add_argument(
        "--bnb-4bit-quant-type",
        type=str,
        choices=("nf4", "fp4"),
        default="fp4",
        help="4-bit quantization type. Defaults to fp4.",
    )
    parser.add_argument(
        "--max-models",
        type=int,
...
def build_quantization_config(args, compute_dtype):
    if args.quantize_bit == 8:
...
def build_device_map(device: str):
    if device.startswith("cuda"):
        return {"": device}
    return None


def validate_device_for_quantization(device: str):
    if not device.startswith("cuda"):
        raise ValueError(
            "BitsAndBytes 4-bit/8-bit quantization requires a CUDA device. "
            "Specify --device cuda:<index> or run on a CUDA-enabled machine."
        )
...
def main():
    ...
    print(f"Quantization: {args.quantize_bit}-bit")
    ...
    for model_name in target_models:
        try:
            ...
            all_results[model_name] = calc_logp(
                tokenizer,
                model,
                texts,
            )
